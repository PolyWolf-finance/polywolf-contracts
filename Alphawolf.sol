...
// WolfToken with Governance.
contract WolfToken is BEP20('PolyWolf Token', 'WOLF') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (AlphaWolf).
...
// AlphaWolf is the master of Wolf. He can make Wolf and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once WOLF is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WOLFs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWolfPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accWolfPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. WOLFs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that WOLFs distribution occurs.
        uint256 accWolfPerShare;   // Accumulated WOLFs per share, times 1e12. See below.
    ...
    // Update the given pool's WOLF allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending WOLFs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wolfReward = multiplier.mul(wolfPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wolf.mint(devaddr, wolfReward.div(10));
        wolf.mint(address(this), wolfReward);
        pool.accWolfPerShare = pool.accWolfPerShare.add(wolfReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to AlphaWolf for WOLF allocation.
    ...
    // Withdraw LP tokens from AlphaWolf.
    ...
    // Safe wolf transfer function, just in case if rounding error causes pool to not have enough WOLFs.
    ...
    function updateEmissionRate(uint256 _wolfPerBlock) public onlyOwner {
        massUpdatePools();
        wolfPerBlock = _wolfPerBlock;
    }
